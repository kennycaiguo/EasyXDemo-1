...
// 完成日期：2016-7-16
// EasyX 版本：2015-10-15
// https://codebus.cn/contributor/a/prismtank-bfs-pathfinding
//
#include <graphics.h>
#include <stdio.h>
#include <memory.h>
...
void map_search(int targx, int targy)
{
	if (targx >= 0 && targy >= 0 && targx < WIDTH && targy < HEIGHT && !map[targx][targy].searched)
	{
		...
		if (targx == endx && targy == endy)
		{
			...
			MessageBox(GetHWnd(), (LPCWSTR)jieguo, (LPCWSTR)text, 0);
		}
		if (targx + 1 < WIDTH)
		{
...
void runxunlu()
{
	...
	if (!success)
		MessageBox(GetHWnd(), (LPCWSTR)"很遗憾,寻路失败", (LPCWSTR)"提示", 0);
...
void render()
{
	...
	outtextxy(WIDTH * 20 + 10, 10, (LPCWSTR)"设起点");

	fillrectangle(WIDTH * 20, 60, WIDTH * 20 + 160, 90);
	outtextxy(WIDTH * 20 + 10, 70, (LPCWSTR)"设终点");

	fillrectangle(WIDTH * 20, 120, WIDTH * 20 + 160, 150);
	outtextxy(WIDTH * 20 + 10, 130, (LPCWSTR)"设障碍");

	fillrectangle(WIDTH * 20, 180, WIDTH * 20 + 160, 210);
	outtextxy(WIDTH * 20 + 10, 190, (LPCWSTR)"开始寻路");

	fillrectangle(WIDTH * 20, 240, WIDTH * 20 + 160, 270);
	outtextxy(WIDTH * 20 + 10, 250, (LPCWSTR)"地图重置");

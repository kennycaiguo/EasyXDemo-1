...
// https://codebus.cn/contributor/a/mengxin-lonely
...
int main()
{
	...
	::GetTempPath(_MAX_PATH, (LPWSTR)tmpmid);
	strcat(tmpmid, "playmid_background.mid");
	ExtractResource((LPWSTR)tmpmid, _T("MUSIC"), _T("1.mid"));

	sprintf(s, "open \"%s\" alias mymusic", tmpmid);
	mciSendString((LPWSTR)s, NULL, 0, NULL);

...
// 最后修改：2014-5-26
// https://codebus.cn/yangw/a/avoid-white
// 本来只能在VC6上运行，我改过了（wangby）
//
#undef UNICODE
#undef _UNICODE
#include <graphics.h>
...
public:
	PLAYER(string name, string keys, int offsetx, int offsety);		// 构造函数
...
PLAYER::PLAYER(string name, string keys, int offsetx, int offsety)
{
	m_strName = name;
	m_keys = keys;
	m_offset.x = offsetx;
...
void PLAYER::DrawFrame()
{
	...
	int w = textwidth(m_strName.c_str());
	OutTextXY((244 - w) / 2, 4, m_strName.c_str());
...
void PLAYER::Hit(char key)
{
	...
	{
	case BEGIN:				// 游戏初次开始
		if (strchr(m_keys.c_str(), key) != NULL)
		{
	...
	case RUNNING:			// 游戏运行中
	{
		const char* pdest = strchr(m_keys.c_str(), key);
		byte pos;
		if (pdest != NULL)						// 判断是否是当前游戏者按键
		{
			pos = pdest - m_keys.c_str();				// 计算按键对应的位置
